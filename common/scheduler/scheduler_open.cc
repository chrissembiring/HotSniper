...
SchedulerOpen::SchedulerOpen(ThreadManager *thread_manager)
  ...
  // initialize pelt
  pelt = new PELT(numberOfCores, dvfsEpoch);

  initMappingPolicy(Sim()->getCfg()->getString("scheduler/open/logic").c_str());
  initDVFSPolicy(
      Sim()->getCfg()->getString("scheduler/open/dvfs/logic").c_str(), pelt);
  initMigrationPolicy(
 ...
 */
void SchedulerOpen::initDVFSPolicy(String policyName, PELT *pelt) {
  ...
  if (policyName == "off") {
    ...
    // else if (policyName ="XYZ") {... } //Place to instantiate a new DVFS
    // logic. Implementation is put in "policies" package.
  } else {
...
void SchedulerOpen::periodic(SubsecondTime time) {
  ...
  // append pelt data
  if (time.getNS() % 1100 == 0) {
    for (int i = 0; i < numberOfCores; i++) {
      double util = 1100 * performanceCounters->getUtilizationOfCore(i);
      pelt->append(i, util);
    }
  }

  if (time.getNS() % 1000000 == 0) {
    // Error Checking at every 1ms. Can be faster but will have overhead
    // in simulation time.
