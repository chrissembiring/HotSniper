...
def main():
    ...
    for run in runs:
        if has_properly_finished(run):
            ...
            # if len(tasks) > 40:
            #     tasks = tasks[:37] + "..."
            splitted = tasks.split(", ")
            tasks = splitted[0]
            tasks += "-" + str(len(get_active_cores(run)))
            base_tasks = splitted[0].split("-")[0]
            rows.append(
                [
                    get_date(run),
                    config,
                    ...
                    "{:d}".format(len(get_active_cores(run))),
                    base_tasks,
                ]
            )
            ...
            cores.append(rows[-1][6])
            base_tasks_.append(rows[-1][7])
